...
class DBTableCreator:
    ...
    def insert_in_chunks(self, base_str, rows):
...
class DBTitleTableCreator(DBTableCreator):
    ...
    def transform_table_row(self, row, headers):
    ...
    def filter_akas(self, row, headers):
        split_row = row.strip().split('\t')
...
class IMDBDataDump:
    ...
    def fetch_data(self):
        self.fetch_dump()
        return readTSV('data/data_dumps/' + self.fname.replace('.gz', ''))
